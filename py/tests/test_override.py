...
# (2) mimick 2nd cycle normal
ws = create_connection(url)

delta = dmp_exact.diff_main('', 'asdf') # old, new
patches = dmp_exact.patch_make('', delta)
patch_txt = dmp_exact.patch_toText(patches)
new_msg = json.dumps({'clock': [0,0],
                      'edits': patch_txt,
                      'client_id':client_id})
print("client sends [OK]: '%s'" %json.loads(new_msg))
ws.send(new_msg)
res1 = json.loads(ws.recv())
print("client received [OK]: '%s'" %(res1))
ws.close()


# (3) mimick corrupt clock
ws = create_connection(url)

delta = dmp_exact.diff_main('asdf', 'qwerty') # old, new
patches = dmp_exact.patch_make('asdf', delta)
patch_txt = dmp_exact.patch_toText(patches)
new_msg = json.dumps({'clock': [10,-1],
                      'edits': patch_txt,
                      'client_id':client_id})
print("client sends [UNFORESEEN SCENARIO]: '%s'" %json.loads(new_msg))
ws.send(new_msg)
res = json.loads(ws.recv())
